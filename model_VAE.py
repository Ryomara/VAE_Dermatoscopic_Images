...
class VAE(nn.Module):
    def __init__(self, x_dim, z_dim, architecture, distribution, loss):
        ...
        self.loss = loss
        self.activation = nn.Sequential(nn.Softplus())
        ...
        nn.ConvTranspose2d(self.channel2, self.channel2, kernel_size=4, stride=2, padding=1, output_padding=0),
        nn.Upsample(64)
        )
        ...
        nn.Conv2d(256, 16, kernel_size=4, stride=1, padding=2),
        nn.BatchNorm2d(16),
        nn.ELU(),
        nn.Conv2d(16, 16, kernel_size=4, stride=2, padding=2),
        nn.BatchNorm2d(16),
        ...
        nn.BatchNorm2d(self.channel2),
        nn.ELU(),
        nn.Conv2d(self.channel2, self.channel2, kernel_size= 1, stride= 1, padding= 0),
        nn.Conv2d(self.channel2, self.channel2, kernel_size= 3, stride= 3, padding= 0),
        nn.ConvTranspose2d(self.channel2, self.channel2, kernel_size= 3, stride= 3, padding= 0),
        nn.Conv2d(self.channel2, self.channel2, kernel_size= 1, stride= 1, padding= 0)
        )
    ...
    def encode(self, x):
        ...
        if self.arch == 'fcn':
            ...
            return output[:, 0, :z_sqrt, :z_sqrt], self.activation(output[:, 1, :z_sqrt, :z_sqrt])
